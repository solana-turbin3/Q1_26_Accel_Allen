...
use mpl_core::{
    ...
    types::{Attribute, Attributes, BurnDelegate, FreezeDelegate, Plugin, PluginAuthority, PluginType, UpdateAuthority}
};
...
impl<'info> Stake<'info> {
    pub fn stake(&mut self, bumps: &StakeBumps) -> Result<()> {
        ...
        // Add BurnDelegate plugin if not already present (for burn-to-earn)
        match fetch_plugin::<BaseAssetV1, BurnDelegate>(&self.nft.to_account_info(), PluginType::BurnDelegate) {
            Err(_) => {
                AddPluginV1CpiBuilder::new(&self.mpl_core_program.to_account_info())
                    .asset(&self.nft.to_account_info())
                    .collection(Some(&self.collection.to_account_info()))
            ...
            Ok(_) => {} // Already has BurnDelegate
        }

        // Update collection-level total_staked attribute
        self.increment_collection_total_staked(signer_seeds)?;

        Ok(())
    }

    fn increment_collection_total_staked(&self, signer_seeds: &[&[u8]]) -> Result<()> {
        match fetch_plugin::<BaseCollectionV1, Attributes>(&self.collection.to_account_info(), PluginType::Attributes) {
            Err(_) => {
                // Add Attributes plugin to collection with total_staked = 1

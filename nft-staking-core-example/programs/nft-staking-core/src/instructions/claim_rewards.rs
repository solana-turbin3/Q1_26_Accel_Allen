...
impl<'info> ClaimRewards<'info> {
    pub fn claim_rewards(&mut self, bumps: &ClaimRewardsBumps) -> Result<()> {
        ...
        // Fetch attributes and verify staked
        ...
        for attribute in &fetched_attribute_list.attribute_list {
            match attribute.key.as_str() {
                ...
                "staked_at" => {
                    ...
                    // Reset to current time to avoid double-claiming
                    attribute_list.push(Attribute {
        ...
        // With freeze_period=0, staked_time_days may be 0 (no rewards to mint, but no error)

        // Update staked_at to current time (reset reward accumulation)
